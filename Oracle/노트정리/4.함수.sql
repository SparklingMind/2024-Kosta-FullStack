...
SELECT ENAME ,UPPER(ENAME), LOWER(ENAME), INITCAP(ENAME), LENGTH(ENAME), LENGTHB(ENAME)
 ...
 SELECT '희정', LENGTH('희정') , LENGTHB('희정') FROM DUAL; 
...
SELECT 이메일, SUBSTR(이메일, 1 ,INSTR(이메일,'@')-1 )  as 앞 , 
...
*/
select sysdate , to_char(sysdate,'YYYY-mm') ,
to_char( sysdate + 2/24 , 'YYYY-MM-DD HH:MI:SS') from dual;
...
-- 입사일 -> 2002년 10월 2일 
SELECT HIREDATE, TO_CHAR(HIREDATE, 'YYYY') || '년', 
CONCAT(TO_CHAR(HIREDATE, 'MM'), '월')
  FROM EMP;


SELECT HIREDATE, TO_CHAR(HIREDATE, '"년"MM"월"DD"일"')
FROM EMP;
...
SELECT SYSDATE - TO_DATE( '2024-1-23') FROM DUAL;
  
-- 100일
SELECT TO_DATE('2023-1-23') + 100
FROM DUAL;
...
/* NULL에 관련된 함수 : NVL(), NVL2(), NULLIF() , COALESCE()
   1) NVL(대상,변경값) : 대상에서 NULL있으면 변경값으로 대치  
   2) NVL2(대상, NULL아닌경우, NULL인경우 ) 
       ex) MGR이 NULL이 아니면 두번째 인수를, NULL이면 세번째인수 리턴
      
   3) NULLIF는 첫번째 인수와 두번째 인수가 같으면 NULL, 아니면 첫번째인수의 값 
   4) COALESCE함수 ; 가장 먼저 NULL이 아닌 것을 반환
 */
...
FROM EMP;
 --NVL2함수 이용
SELECT ENAME, COMM, NVL2(COMM, COMM*12, 30) , NVL(COMM, 30) FROM EMP;

--NULLIF는 첫번째 인수와 두번째 인수가 같으면 NULL, 아니면 첫번째인수의 값  
...
SELECT * FROM REPORT;
 ...
 --DECODE
 SELECT BAN , NAME, KOR , 
 DECODE(BAN,1,'MSA과정',2,'IOT과정',3,'DESIGN과정', 'FULL STACK과정') AS 과정명
 FROM REPORT;
